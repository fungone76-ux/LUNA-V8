...
# Every key in a stage's llm_context must be listed here.
# If a new key is added to a YAML but not handled in _build_context(), it will be silently
# ignored — which has caused bugs before (forbidden_in_this_scene went unhandled for weeks).
_KNOWN_LLM_CONTEXT_KEYS: frozenset = frozenset({
    "scene", "tone", "pacing", "explicit_permission",
    "forbidden_in_this_scene", "voyeur_mechanic",
    "dual_character", "outfit_note",
})
    ...
    def _eval(self, c: QuestCondition, gs: GameState, user_input: str) -> bool:
        ...
        if t == "player_action":
            ...
            if not pattern:
                return False
            import re as _re
            # user_input è il parametro corrente (turno in corso); il flag è fallback
            # per compatibilità con chiamate che non passano user_input direttamente
            player_input = user_input or gs.flags.get("_last_player_input", "")
...
class QuestEngine:
    ...
    def _execute_action(
        ...
        if act == "set_flag":
        ...
        elif act == "set_npc_location":
            npc_id = str(char or "")
            loc = str(value or "") if value else game_state.current_location
            if npc_id and loc:
                game_state.set_npc_location(npc_id, loc)
                logger.info("[QuestAction] set_npc_location → %s at %s", npc_id, loc)

        elif act == "snapshot_affinity":
            companion_raw = char or game_state.active_companion
            companion = companion_raw
            if engine:
                companions = getattr(getattr(engine, "world", None), "companions", {})
                ...
                companion = next((k for k in companions if k.lower() == lower), companion_raw)
            current = game_state.affinity.get(companion, 0)
            game_state.flags[f"_aff_snap_{companion}"] = current
            logger.info("[QuestAction] snapshot_affinity %s = %d", companion, current)

        elif act == "restore_affinity_snapshot":
            companion_raw = char or game_state.active_companion
            companion = companion_raw
            if engine:
                companions = getattr(getattr(engine, "world", None), "companions", {})
                lower = companion_raw.lower().strip()
                companion = next((k for k in companions if k.lower() == lower), companion_raw)
            snapshot = int(game_state.flags.pop(f"_aff_snap_{companion}", game_state.affinity.get(companion, 0)))
            delta = int(value) if value is not None else 0
            new_val = max(0, min(100, snapshot + delta))
            game_state.affinity[companion] = new_val
            logger.info("[QuestAction] restore_affinity_snapshot %s: %d + %d = %d", companion, snapshot, delta, new_val)
            ...
            emotional_impact = (
                action.emotional_impact if hasattr(action, "emotional_impact")
                else action.get("emotional_impact", "neutral")
                if isinstance(action, dict) else "neutral"
            )
            if companion and entry_text:
                game_state.flags[f"_pending_npc_memory_{companion}"] = {
                    "entry": entry_text,
                    "emotional_impact": emotional_impact,
                }
                logger.info("[QuestAction] write_memory queued for '%s'", companion)
    ...
    def _build_context(self, game_state: GameState) -> Tuple[str, str]:
        ...
        for quest_id in game_state.active_quests:
            ...
            if stage.llm_context:
                ...
                for _unknown_key in ctx:
                    if _unknown_key not in _KNOWN_LLM_CONTEXT_KEYS:
                        logger.warning(
                            "[QuestEngine] Unknown llm_context key '%s' in %s/%s — "
                            "add a handler in _build_context() or it will be ignored",
                            _unknown_key, quest_id, stage_id,
                        )
                if ctx.get("scene"):
                ...
                if ctx.get("forbidden_in_this_scene"):
                    parts.append(
                        f"=== SCENE PROHIBITION (MANDATORY) ===\n"
                        f"⚠️ FORBIDDEN in this scene — do NOT mention or introduce:\n"
                        f"{ctx['forbidden_in_this_scene']}\n"
                        f"Violation of this rule breaks scene coherence. IGNORE any story beat or NPC state that contradicts it."
                    )
    ...
    def _find_eligible_quests(
        ...
        for quest_id, quest_def in self.world.quests.items():
            ...
            if quest_def.activation_type == "event":
                ...
                if quest_def.trigger_location and game_state.current_location != quest_def.trigger_location:
                    continue
                if quest_def.allowed_times:
                    current_time = game_state.time_of_day.value if hasattr(game_state.time_of_day, "value") else str(game_state.time_of_day)
                    if current_time not in quest_def.allowed_times:
                ...
                watch_flag = quest_def.trigger_flag
                if not watch_flag:
                    continue
                flag_set_turn = game_state.flags.get(f"_flag_ts_{watch_flag}")
                if flag_set_turn is None:
                    continue
                days_elapsed = (game_state.turn_count - int(flag_set_turn)) / 4  # 4 fasi = 1 giorno
                if days_elapsed < quest_def.trigger_days:
                    continue
                if not self._evaluator.evaluate_all(quest_def.activation_conditions, game_state, user_input):
                    continue
                ...
                if not initiative_npc:
                    continue
                if quest_def.character and initiative_npc.lower() != quest_def.character.lower():
                    continue
                if quest_def.allowed_times:
                    current_time = game_state.time_of_day.value if hasattr(game_state.time_of_day, "value") else str(game_state.time_of_day)
                    if current_time not in quest_def.allowed_times:
            ...
            elif quest_def.activation_type in ("auto", "trigger", "choice"):
                if quest_def.allowed_times:
                    current_time = game_state.time_of_day.value if hasattr(game_state.time_of_day, "value") else str(game_state.time_of_day)
                    if current_time not in quest_def.allowed_times:
                        continue
    ...
    def _activate_quest(
        ...
        if quest_def.requires_player_choice:
            ...
            # For random quests, set cooldown immediately on activation so they
            # don't re-trigger on the next turn before the scene even ends.
            if quest_def.activation_type == "random" and quest_def.cooldown_turns > 0:
                game_state.flags[f"_cooldown_{quest_id}"] = (
                    game_state.turn_count + quest_def.cooldown_turns
                )
                logger.info(
                    "[QuestEngine] Random quest '%s' cooldown set: %d turns",
                    quest_id, quest_def.cooldown_turns,
                )
    ...
    def _update_active_quest(
        ...
        # Player-action exits bypass the location gate: the player's dialogue
        # (e.g. "arrivederci") must be evaluated regardless of current position,
        # because the player may have already moved away by the time we check.
